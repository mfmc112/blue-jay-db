...
public class SearchAggregatorTest {
	...
	@Test
	public void aggregateAndMultyTest() {
	...
	public void aggregateAnd100kTime() throws FCException {
		...
		Set<Long> set2 = randomSet(70000, 100000);
		log.startTimer();
		SearchAggregator s = new SearchAggregator();
		...
		// It needs to run in less then 100 milliseconds for 100k. Yes 1/10 of a second
		Assert.assertTrue(ended <=100);
		Assert.assertEquals(30000, aggregatedSet.size());
	}
	
	@Test
	public void aggregateOrMultyTest() {
		Long[] l1 = new Long[]{2L,4L,6L,8L};
		Set<Long> set1 = new HashSet<Long>(Arrays.asList(l1));
		...
		SearchAggregator s = new SearchAggregator();
		Set<Long> aggregatedSet = s.aggregateOr(set1, set2, set3);

		Assert.assertEquals(7, aggregatedSet.size());
		Assert.assertTrue(aggregatedSet.contains(1L));
		Assert.assertTrue(aggregatedSet.contains(2L));
		Assert.assertTrue(aggregatedSet.contains(3L));
		Assert.assertTrue(aggregatedSet.contains(4L));
		Assert.assertTrue(aggregatedSet.contains(5L));
		Assert.assertTrue(aggregatedSet.contains(6L));
		Assert.assertTrue(aggregatedSet.contains(8L));
		Assert.assertFalse(aggregatedSet.contains(7L));
	}
	
	@Test
	public void aggregateOrSingleSetTest() {
		Long[] l1 = new Long[]{2L,4L,6L,8L};
		Set<Long> set1 = new HashSet<Long>(Arrays.asList(l1));
		SearchAggregator s = new SearchAggregator();
		Set<Long> aggregatedSet = s.aggregateOr(set1);
	...
	@Test
	public void aggregateOr100kTime() throws FCException {
		Set<Long> set1 = randomSet(0, 100000);
		Set<Long> set2 = randomSet(70000, 100000);
		log.startTimer();
		SearchAggregator s = new SearchAggregator();
